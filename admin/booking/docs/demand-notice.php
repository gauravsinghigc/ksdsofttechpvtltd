...
<body onload="doConvert()" style=" font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI' , Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans' , 'Helvetica Neue' , sans-serif !important;margin-left:1.5rem;">
  <section style="padding:0.5rem;margin:2% auto;display:block;">
    ...
    <?php } ?> -->
      ...
      <p><b>Subject:</b> Final Demand notice cum offer of possession for your <b>Plot No : <?php echo $numbersOnly; ?></b> in the Project-
        <b>"<?php echo FETCH($BookingSql, "project_name"); ?>"</b> situated at Sector-5. Pataudi, Gurugram, Haryana – 122503.
      </p>
      ...
      <p style="text-align: justify;">
        ...
        the project <b>"<?php echo FETCH($BookingSql, "project_name"); ?>"</b> has been granted in respect of license <b>bearing no. 94 of 2017
      ...
      <ol>
        <li>After completion of construction, the Final Area of the said <b>Plot bearing no : <?php echo $numbersOnly; ?></b>
          is <b><?php echo $unit_area; ?></b>. </li>
        <li>
          <p style="text-align:justify;">
            As per the Payment Plan, instalment of <b><?php echo Price($BookingBalance = FETCH($BookingSql, "net_payable_amount") - $net_paid_amount2, "", "Rs."); ?></b>/- <b>(<?php echo PriceInWords($BookingBalance); ?>)</b> out of the Basic Selling Price
            <b>(hereinafter referred to as “BSP”)</b>, towards the final payments due against <b>Plot No : <?php echo $numbersOnly; ?></b> is payable at your end in order to enable us to start the process of
            handing over the possession of your unit. Kindly note that the due date for the
            payment of the due amount is <b><?php echo $DueDate =  IfRequested("GET", "due_date", date("d M, Y", strtotime("+15 days")), false); ?></b>.
          </p>
        </li>
        <li>
          <p style="text-align:justify;">
            Further, the payments against <b>“Annexure-A”</b> of the Application Form and
            <b>Schedule “C”</b> of the Plot Buyer Agreement <b>(hereinafter referred to as “PBA)</b>, executed by you and KSD Buildtech Pvt Ltd, are now pending. Total dues at
    ...
    <div style="font-size:14px !important;">
      ...
      <table style="width:100%;" border="1">
        ...
        if ($_GET['IFMS'] != 0) {
          ...
          <tr>
            ...
            <td>Additional Cost: Interest Free Maintenance
              ...
              <b>(<?php echo Price($IFMSPrice, "", "Rs."); ?> x <?php echo GetNumbers($unit_area); ?> sq. yards = <?php echo Price($IFMSPrice * GetNumbers($unit_area), "", "Rs."); ?>/-)</b>
            </td>
            <th><?php echo Price($IFMSCharges = $IFMSPrice * GetNumbers($unit_area), "", "Rs."); ?>/-</th>
          </tr>
        <?php
        } else {
          $PointNo = 6;
        ...
        ?>
          <tr>
            <th><?php echo $PointNo = $PointNo + 1; ?></th>
            <td>1 Year advance upkeep charges <b>@ <?php echo Price($UPKEEPPrice, "", "Rs."); ?></b> per
              sq. yards.<br>
              <b><?php echo "Rs.$UPKEEPPrice x " . Getnumbers($unit_area) . " sq. yards = Rs." . ($UPKEEPPrice * GetNumbers($unit_area)) * 12; ?>/-</b>
            </td>
            <th><?php echo Price($UPKEEPCHarge = ($UPKEEPPrice * GetNumbers($unit_area)) * 12, "", "Rs."); ?>/-</th>
          </tr>
        <?php
        } else {
          $UPKEEPCHarge = 0;
      ...
      <p>
        <?php if ($IFMSCharges != 0 or $UPKEEPCHarge != 0) { ?>
          <br><br>
          <b>NOTE: </b><br>
        <?php  } else {
          echo "<br><br><br><br>";
        ...
        if ($IFMSCharges != 0) { ?>
          <b>* Additional Cost:</b> Interest Free Maintenance Security (IFMS)= <b>Rs.<?php echo $IFMSPrice; ?></b>/- x <b><?php echo GetNumbers($unit_area); ?> Sq. Yard</b> (Area of the allotted unit) = <b><?php echo Price($IFMSCharges, "", "Rs."); ?></b>/- <b>(<?php echo PriceInWords($IFMSCharges); ?>)</b>
          <br><br>
        <?php }
        if ($UPKEEPCHarge != 0) { ?>
          <b>* 1 Year advance upkeep charges : </b> <b><?php echo Price($UPKEEPPrice, "", "Rs."); ?></b> x <b><?php echo Getnumbers($unit_area); ?> sq. yards</b> (Area of the
          allotted unit) = <b><?php echo Price($UPKEEPCHarge, "", "Rs."); ?></b>/- <b>(<?php echo PriceInWords($UPKEEPCHarge); ?>)</b>.
          <br>
        <?php } ?>
      </p>
      <p style='text-align:justify;'>
        ...
        your property shall be as follows: <b>Plot No : <?php echo $numbersOnly; ?></b> in the Project-<b>"<?php echo FETCH($BookingSql, "project_name"); ?>"</b>
  ...
  <section style="height:100%;padding:0.5rem;margin:2% auto;display:block;">
    ...
    <div style="font-size:14px !important;">
      <ol start="5">
        ...
        <li>
          <p style='text-align:justify;'><b>Holding Charges:</b> Please be aware that upkeep charges of <b>Rs. <?php echo $UPKEEPPrice; ?></b> per square
        ...
        <li>
          <p style="text-align:justify">
            ...
            Pvt Ltd that while the statement of accounts reflects the total dues of <B><?php echo Price($NET_PAYABLE_AS_PER_FINAL_DEMAND_NOTICE, "", "Rs."); ?></B> - as on <b><?php echo $DueDate; ?></b>, EDC/IDC charges which are levied by the
    ...
    <div style="font-size:14px !important;">
      ...
      <table style='width:100%;' border="1">
        ...
        <tr>
          ...
          <th><?php echo FETCH("SELECT * FROM user_documents where document_name like '%PAN CARD%' and user_id='$customer_id'", "user_documents_no"); ?></th>
        </tr>
        <tr>
          <th>2.</th>
